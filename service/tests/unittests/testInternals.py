...
class TestHelperFunctions(TestCase):
  ...
  def test_mock_query(self):
    ...
    expected_attribs = ['_sa_instance_state', 'centroid', 'cluster', 'members']
    ...
    self.assertEqual(sorted(resp.__dict__.keys()), sorted(expected_attribs))
    # Querying the mock for all records should return a lists of Clusters
    res = db.session.query(Clusters).all()
    self.assertEqual([x.__class__.__name__ for x in res],['Clusters','Clusters'])
...
class TestDataRetrieval(TestCase):
  ...
  def test_normalized_keywords(self):
    ...
    # When normalized keywords from the AST collection are supplied, they should be returned
    ...
    # When other keywords (i.e. not in AST collection) are supplied, nothing gets returned (Error)
...
class TestArticleDataRetrieval(TestCase):
  ...
  def test_article_data(self):
    ...
    # By default this method should return a list of dictionaries that includes references
    expected_keywords = ["aberration","ablation","absorption"]
    mockdata = [
    {'id':'1','bibcode':'a','title':['a_title'],'first_author':'a_author','reference':['x','z']},
    {'id':'2','bibcode':'b','title':['b_title'],'first_author':'b_author','reference':['d','x']},
    ]
    ...
    expected = [{u'first_author': u'a_author', u'bibcode': u'a', u'id': u'1', u'reference': [u'x', u'z'], u'title': [u'a_title']}, {u'first_author': u'b_author', u'bibcode': u'b', u'id': u'2', u'reference': [u'd', u'x'], u'title': [u'b_title']}]
    self.assertEqual(results, expected)

    # When 'check_references=False' is supplied, we should get a dictionary with authors and titles
    results = get_article_data('bibcode', check_references=False)
    expected = {u'a': {'author': u'a_author,+', 'title': u'a_title'}, u'b': {'author': u'b_author,+', 'title': u'b_title'}}
    self.assertEqual(results, expected)
...
class TestCitationDataRetrieval(TestCase):
  ...
  def test_citation_data(self):
    ...
    # This method should return a dictionary where the key 'Results' holds all the citations
    # for the input bibcodes
    expected_keywords = ["aberration","ablation","absorption"]
    mockdata = [
    {'id':'1','bibcode':'a','title':['a_title'],'first_author':'a_author','citation':['x','z']},
    {'id':'2','bibcode':'b','title':['b_title'],'first_author':'b_author','citation':['d','x']},
    ]
...
class TestVectorCreation(TestCase):
  ...
  def test_create_paper_vector(self):
    ...
    # For a paper with AST normalized keywords, we should get a non-trivial vector
    expected_keywords = ["aberration","ablation","absorption"]
    mockdata = [{'id':'1','bibcode':'a','keyword_norm':expected_keywords + ["x","y","z"]}]
    ...
    # By design we have only the first three normalized keywords: only the first
    ...
    # If a paper has no keywords or no normalized keywords, an error is returned
    mockdata = [{'id':'1','bibcode':'a','keyword_norm':["x","y","z"]}]
...
class TestProjection(TestCase):
  ...
  def test_project_paper(self):
    '''Test to see if paper vector gets properly projected into a given cluster'''
    from utils.recommender import project_paper
    # First we test projecting a paper in the 100-dimensional space, which corresponds
    # with cluster "-1". This projection happens in the beginning, just after we established
    # the initial paper vector based on its normalized keywords
    vector = [0.0]*993
    vector[0] = 1/float(3)
    vector[1] = 1/float(3)
    vector[2] = 1/float(3)
    ...
    np.testing.assert_almost_equal(sum(list(cluster_vector)), 0.00166657306484, 10)

    # Now we test a project of a paper within a specific cluster, i.e. from 100 to 5 dimensions
    vector = [0.0]*100
    vector[0] = 1/float(3)
    vector[1] = 1/float(3)
    vector[2] = 1/float(3)
    ...
    self.assertTrue(all(isinstance(x,float) for x in cluster_vector))
    self.assertTrue(type(cluster_vector).__module__ == np.__name__)
    expected = np.array([ 0.37929732, -0.07042716,  0.10629903,  0.11877967, -0.13284801])
  ...
  def test_find_paper_cluster(self):
    ...
    # If we have a paper that is already in a cluster, the 'one' method should find it
    # Paper 'a' is in cluster 1
    bibcode = 'a'
    cluster = find_paper_cluster(pvec,bibcode)
    self.assertTrue(cluster==1)
    # Paper 'z' is in no cluster, but it's vector is closest to the centroid of cluster 2
    bibcode = 'z'
    cluster = find_paper_cluster(pvec,bibcode)
  ...
  def test_find_closest_cluster_papers(self):
    ...
    # Since our test paper has the exact same vector as 'paper_3' in the mock data
    # the returned list should have length 1 with 'paper_3' as its entry
  ...
  def test_find_recommendations(self):
    ...
    mockdata = [
    {'id':'1','bibcode':'ppr1','reference':['r1','r2'],'citation':['c1'],'citation_count':1},
    {'id':'2','bibcode':'ppr2','reference':['r2','r3'],'citation':['c1','c2','c3'],'citation_count':3},
    {'id':'2','bibcode':'ppr3','reference':['r2','r3'],'citation':['c2','c3'],'citation_count':2}
    ]
    ...
    expected_recommendations = ['Field definitions:', 'paper_3', 'ppr2', 'ppr3', 'ppr2', u'ppr1', u'c1', u'r2', u'ppr2']
  ...
  def test_everything(self):
    ...
    expected_recommendations = {'paper': 'a', 'recommendations': [{'bibcode': 'ppr3', 'author': u'au_ppr3,+', 'title': u'ttl_ppr3'}, {'bibcode': 'ppr2', 'author': u'au_ppr2,+', 'title': u'ttl_ppr2'}, {'bibcode': u'ppr1', 'author': u'au_ppr1,+', 'title': u'ttl_ppr1'}, {'bibcode': u'c1', 'author': u'au_c1,+', 'title': u'ttl_c1'}, {'bibcode': u'r2', 'author': u'au_r2,+', 'title': u'ttl_r2'}, {'bibcode': u'ppr2', 'author': u'au_ppr2,+', 'title': u'ttl_ppr2'}]}
